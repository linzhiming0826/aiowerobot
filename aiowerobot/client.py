...
from aiowerobot.utils import to_text
from aiowerobot.replies import Article
...
def check_error(content):
    ...
    if "errcode" in content and content["errcode"] != 0:
        raise ClientException("{}: {}".format(
            content["errcode"], content["errmsg"]))
    return content
...
class Client(object):
    ...
    def __init__(self, config, _token=None, token_expires_at=None):
        self.config = config
        self._token = _token
    ...
    async def get_ip_list(self):
        """
        获取微信服务器IP地址。

        :return: 返回的 JSON 数据包
        """
        return await self.get(url="https://api.weixin.qq.com/cgi-bin/getcallbackip")
    ...
    async def get_menu(self):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self.get("https://api.weixin.qq.com/cgi-bin/menu/get")
    ...
    async def delete_menu(self):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self.get("https://api.weixin.qq.com/cgi-bin/menu/delete")

    async def create_custom_menu(self, menu_data, matchrule):
        """
        创建个性化菜单::
    ...
    async def delete_custom_menu(self, menu_id):
        """
        删除个性化菜单。
    ...
    async def match_custom_menu(self, user_id):
        """
        测试个性化菜单匹配结果。
    ...
    async def get_custom_menu_config(self):
        """
        获取自定义菜单配置接口。
    ...
    async def add_custom_service_account(self, account, nickname, password):
        """
        添加客服帐号。
    ...
    async def update_custom_service_account(self, account, nickname, password):
        """
        修改客服帐号。
    ...
    async def delete_custom_service_account(self, account, nickname, password):
        """
        删除客服帐号。
    ...
    async def upload_custom_service_account_avatar(self, account, avatar):
        """
        设置客服帐号的头像。
    ...
    async def get_custom_service_account_list(self):
        """
        获取所有客服账号。
    ...
    async def get_online_custom_service_account_list(self):
        """
        获取状态为"在线"的客服账号列表。
    ...
    async def download_media(self, media_id):
        ...
        :return: aiohttp 的 Response 原始数据
        """
        async with aiohttp.request(method='GET', url="https://api.weixin.qq.com/cgi-bin/media/get", params={"access_token": self._token, "media_id": media_id}) as r:
            return await r.read()

    async def add_news(self, articles):
        """
        新增永久图文素材::
    ...
    async def upload_news_picture(self, file):
        """
        上传图文消息内的图片。
    ...
    async def upload_permanent_media(self, media_type, media_file):
        """
        上传其他类型永久素材。
    ...
    async def upload_permanent_video(self, title, introduction, video):
        """
        上传永久视频。
        ...
        async with aiohttp.request(method='POST',
                                   ...
                                   params={"access_token": self._token,
                                           "type": "video"},
                                   data={'media': video, "description": json.dumps(
                                       {"title": title, "introduction": introduction}, ensure_ascii=False).encode("utf-8")}) as r:
            return await r.text()

    async def download_permanent_media(self, media_id):
        """
        获取永久素材。

        :param media_id: 媒体文件 ID
        :return: aiohttp 的原始数据
        """
        async with aiohttp.request(method='POST',
                                   url="https://api.weixin.qq.com/cgi-bin/material/get_material",
                                   params={"access_token": self._token},
                                   data=json.dumps({"media_id": media_id}, ensure_ascii=False).encode("utf-8")) as r:
            return await r.read()

    async def delete_permanent_media(self, media_id):
        """
        删除永久素材。
    ...
    async def update_news(self, update_data):
        """
        修改永久图文素材::
    ...
    async def get_media_list(self, media_type, offset, count):
        """
        获取素材列表。
    ...
    async def get_groups(self):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self.get("https://api.weixin.qq.com/cgi-bin/groups/get")

    async def get_group_by_id(self, openid):
        """
        查询用户所在分组。
    ...
    async def move_users(self, user_id_list, group_id):
        """
        批量移动用户分组。
    ...
    async def remark_user(self, user_id, remark):
        """
        设置备注名。
    ...
    async def get_user_info(self, user_id, lang="zh_CN"):
        """
        获取用户基本信息。
    ...
    async def get_users_info(self, user_id_list, lang="zh_CN"):
        """
        批量获取用户基本信息。
    ...
    async def send_text_message(self, user_id, content, kf_account=None):
        """
        发送文本消息。
    ...
    async def send_image_message(self, user_id, media_id, kf_account=None):
        """
        发送图片消息。
    ...
    async def send_voice_message(self, user_id, media_id, kf_account=None):
        """
        发送语音消息。
    ...
    async def send_music_message(
        ...
        对此我们也无能为力 ( `#197 <https://github.com/whtsky/AioWeRoBot/issues/197>`_ )
    ...
    async def send_article_message(self, user_id, articles, kf_account=None):
        """
        发送图文消息::
    ...
    async def send_news_message(self, user_id, media_id, kf_account=None):
        """
        发送永久素材中的图文消息。
    ...
    async def send_miniprogrampage_message(
        self,
        user_id,
        title,
        appid,
    ...
    async def create_qrcode(self, data):
        """
        创建二维码。
    ...
    async def show_qrcode(self, ticket):
        """
        通过ticket换取二维码。

        :param ticket: 二维码 ticket 。可以通过 :func:`create_qrcode` 获取到
        :return: 返回的 aiohttp 原始数据
        """
    ...
    async def get_tags(self):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self.get(url="https://api.weixin.qq.com/cgi-bin/tags/get", )
    ...
    async def get_users_by_tag(self, tag_id, next_open_id=""):
        """
        获取标签下粉丝列表
    ...
    async def get_tags_by_user(self, open_id):
        """
        获取用户身上的标签列表
    ...
    async def tag_users(self, tag_id, open_id_list):
        """
        批量为用户打标签
    ...
    async def untag_users(self, tag_id, open_id_list):
        """
        批量为用户取消标签
    ...
    async def upload_news(self, articles):
        """
        上传图文消息素材 ::
    ...
    async def send_mass_msg(
        ...
        return await self.post(url=url, data=send_data)

    async def delete_mass_msg(self, msg_id, article_idx=0):
        """
        群发之后，随时可以通过该接口删除群发。
    ...
    async def send_mass_preview_to_user(
    ...
    async def get_mass_msg_status(self, msg_id):
        """
        查询群发消息发送状态。
    ...
    async def get_mass_msg_speed(self):
        """
        获取群发速度。
    ...
    async def set_mass_msg_speed(self, speed):
        """
        设置群发速度。
