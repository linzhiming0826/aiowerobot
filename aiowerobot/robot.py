...
class BaseRoBot(object):
    ...
    :param logger: 用来输出 log 的 logger，如果是 ``None``，将使用 aiowerobot.logger
    :param config: 用来设置的 :class:`aiowerobot.config.Config` 对象 \\

    .. note:: 对于下面的参数推荐使用 :class:`~aiowerobot.config.Config` 进行设置，\
    并且以下参数均已 **deprecated**。
    ...
    将使用 aiowerobot.session.sqlitestorage.SQLiteStorage **(deprecated)**
    ...
    def __init__(
        ...
        **kwargs
        ...
        if logger is None:
            import aiowerobot.logger
            logger = aiowerobot.logger.logger
        self.logger = logger
    ...
    def parse_message(
        ...
        """
        解析获取到的 Raw XML ，如果需要的话进行解密，返回 AioWeRoBot Message。
        :param body: 微信服务器发来的请求中的 Body。
        :return: AioWeRoBot Message
        """
    ...
    async def get_encrypted_reply(self, message):
        """
        对一个指定的 AioWeRoBot Message ，获取 handlers 处理后得到的 Reply。
        如果可能，对该 Reply 进行加密。
        返回 Reply Render 后的文本。

        :param message: 一个 AioWeRoBot Message 实例。
        :return: reply （纯文本）
        """
...
class AioWeRoBot(BaseRoBot):
    ...
    def run(self, host=None, port=None, enable_pretty_logging=True, **kwargs):
        ...
        if enable_pretty_logging:
            from aiowerobot.logger import enable_pretty_logging
